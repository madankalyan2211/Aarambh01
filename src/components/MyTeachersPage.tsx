...
export function MyTeachersPage({ onNavigate, userEmail }: MyTeachersPageProps) {
  ...
  const [selectedTeacherForEnrollment, setSelectedTeacherForEnrollment] = useState(null);
  ...
  const fetchTeachers = async () => {
    ...
    try {
      ...
      console.log('All Teachers Response:', allTeachersRes);
      console.log('My Teachers Response:', myTeachersRes);
      
      if (allTeachersRes.success) {
        console.log('All Teachers Data:', allTeachersRes.data);
        if (allTeachersRes.data && allTeachersRes.data.length > 0) {
          console.log('First teacher sample:', allTeachersRes.data[0]);
          console.log('First teacher _id:', allTeachersRes.data[0]._id);
          console.log('First teacher id:', allTeachersRes.data[0].id);
        }
  ...
  const handleEnrollToggle = async (teacherId, showCourses = false) => {
      ...
      <div className="container mx-auto max-w-7xl">
                  ...
                  {selectedTeacher.teachingCourses && selectedTeacher.teachingCourses.length > 0 ? (
                    <div className="grid md:grid-cols-2 gap-3">
                      {selectedTeacher.teachingCourses.map((course, idx) => (
                        <Card key={idx} className="p-4 bg-gradient-to-br from-[#FF69B4]/5 to-accent/5 border-[#FF69B4]/20">
        ...
        <Dialog open={showCoursesDialog} onOpenChange={setShowCoursesDialog}>
          <DialogContent className="sm:max-w-2xl">
            <DialogHeader>
              <DialogTitle style={{ color: '#FF69B4' }}>
                      ...
                      <Card 
                        ...
                        className={`p-4 cursor-pointer transition-all border-2 ${
                          isSelected 
                            ...
                            : 'border-[#FF69B4]/20 hover:border-[#FF69B4]/50'
                        }`}
